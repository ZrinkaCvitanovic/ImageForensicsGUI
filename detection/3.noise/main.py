...
def MergeBlock1(kwargs):
    ...
    label[label<128] = 0
    label[label>=128] = 255
    cv2.imwrite("prediction1.jpg", label)

    return label

def MergeBlock2(kwargs):
    blockMatrix = kwargs["blk"]
    label       = kwargs["label"]
    label_idx   = kwargs["label_id"]
    HH          = kwargs["HH"]
    ...
    label[label<128] = 0
    label[label>=128] = 255
    cv2.imwrite("prediction2.jpg", label)
...
if __name__ == "__main__":
    ...
    dicts = SplitImages(args.in_path, None)
    label1 = MergeBlock1(dicts)
    label2 = MergeBlock2(dicts)