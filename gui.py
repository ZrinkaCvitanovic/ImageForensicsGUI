...
tk.Button(tab1, text="Browse Files",command=browseFiles).grid(row=1, column=2, sticky="w")
...
tooltip_ela = CustomTooltip(ent_ela_q, "Recommended values for quality are between 40 and 80. For high resolution images, choose lower values and vice versa.")
...
tk.Button(tab1, text="Browse Files",command=browseFiles).grid(row=11, column=2, sticky="w")
...
#ent_color_method = tk.Entry(tab1, width=5)
...
tooltip_color = CustomTooltip(opt_color_dropdown, "HSV works better for cut-paste detection, while LUM can work for copy-move as well.")
...
tk.Button(tab1, text="Browse Files",command=browseFiles).grid(row=21, column=2, sticky="w")
...
lbl_edge_robust = tk.Label(tab1, text="Robust edge detection", width=50).grid(row=24, column=0, sticky="w")

robust_pressed = tk.IntVar()
btn_edge_robust = tk.Checkbutton(tab1, text="", variable=robust_pressed, onvalue=1,
                        offvalue=0).grid(column=1, row=24, sticky="w")
tk.Label(tab1, text="Robust detection causes a fewer amount of detected edges. Useful for highly texturized or noisy images.").grid(column=1, row=25, sticky="w")
...
tooltip_lower = CustomTooltip(ent_edge_lower, "Recommended values for lower threshold are up to 200. For darker pictures, the threshold can be higher.")
ent_edge_higher = tk.Entry(tab1, width=5)
ent_edge_higher.grid(row=23, column=1, sticky="w")
tooltip_higher = CustomTooltip(ent_edge_higher, "Recommended value for higher threshold is at least 3 time larger than the lower threshold. For low contrast images, this threshold can be lower.")
...
tooltip_telea = CustomTooltip(ent_telea_radius, "Default value for radius is 5. This works for most pictures.")
...
tk.Button(tab2, text="Browse Files",command=browseFiles).grid(row=12, column=2, sticky="w")
tk.Label(tab2, text="Mask color", width=50).grid(row=13, column=0, sticky="w")
#tk.Label(tab2, text="Pyramid/single", width=50).grid(row=14, column=0, sticky="w")   TODO
...
#ent_patch_method = tk.Entry(tab2, width=100)                                        TODO
...
tooltip_height = CustomTooltip(ent_resize_width, "Choose how much you want to increase width of the image (enter 1 for no change). It's recommended to make this value equal to height scale.")
...
tk.Button(tab2, text="Browse Files",command=browseFiles).grid(row=31, column=2, sticky="w")
...
tk.Button(tab2, text="Browse Files",command=browseFiles).grid(row=41, column=2, sticky="w")
tk.Label(tab2, text="Method", width=50).grid(row=42, column=0, sticky="w")
...
opt_sharp_dropdown = tk.OptionMenu(tab2, opt_sharp_method, "histogram", "kernel", "laplacian")
