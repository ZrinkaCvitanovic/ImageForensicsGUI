...
def event_ela(event):
    ...
    try:
        ...
        if quality < 0 or quality > 100:
            ...
            subprocess.run(['python', 'detection\\ela\\main.py', in_image, str(quality)])
...
def event_color(event):
    ...
    if method != "lum" and method != "hsv":
        ...
        subprocess.run(['python', 'detection\\change-color-scheme\\main.py', in_image, method])
...
def event_edge(event):
    ...
    try:
        ...
        subprocess.run(['python', 'detection\\edges\\main.py', in_image, str(lower), str(higher)])
...
def event_telea(event):
    ...
    try:
        ...
        subprocess.run(['python', 'restoration\\telea\\main.py', '-r', radius, in_image, mask_image])
...
def event_patch(event):
    ...
    if color != "white" and color != "black" and color != "blue" and color != "green" and color != "red":
        ...
        subprocess.run(['python', 'restoration\\PatchMatch\\main.py', in_image, mask_image, color])
...
def event_noise(event):
    ...
    if method != "gaussian" and method != "median":
        ...
        subprocess.run(['python', 'restoration\\enhancement\\noise\\denoising.py', in_image, method])
...
def event_sharp(event):
    ...
    if method != "kernel" and method != "laplacian":
        ...
        subprocess.run(['python', 'restoration\\enhancement\\sharpen\\sharpen.py', in_image, method])            
        lbl_sharp_msg["text"] ="Success!"
        ent_sharp_in.delete(0, tk.END)

def event_contrast(event):
    in_image = ent_contrast_in.get()
    subprocess.run(['python', 'restoration\\enhancement\\sharpen\\histogram.py', in_image])          
